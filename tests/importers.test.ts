...
test("rejects a ChatGPT export graph with a cycle below the root instead of hanging or crashing", () => {
  ...
  assert.throws(() => importChatGptExport({ title: "cycle", mapping }), /reachable more than once|Inconsistent ChatGPT/);
});

test("preserves every disconnected ChatGPT root", () => {
  const mapping = {
    root: {
      id: "root",
      parent: null,
  ...
  assert.equal(result.record.events.length, 3);
...
test("diagnoses an unresolved Claude parent instead of inventing a relationship", () => {
  assert.throws(() => importClaudeExport({
    uuid: "conv",
...
test("preserves unsupported Claude blocks as source JSON", () => {
  const result = importClaudeExport({
    uuid: "conv",
  ...
  const source = result.record.events[1];
  assert.ok(source?.type === "system_event");
  assert.deepEqual(JSON.parse(source.detail!), { type: "image", source: { type: "base64", data: "..." } });
});
...
test("preserves indentation on the first/last line of a pasted message", () => {
  ...
  assert.equal(textContent?.type === "text" ? textContent.text : undefined, "\n    def foo():\n        pass");
});
...
test("warns when a ZIP export contains files beyond conversations.json", () => {
  ...
  assert.equal(result.detected, "chatgpt_export");
  const source = result.record.events.find(e => e.type === "system_event" && e.label === "Source ZIP file references");
  assert.ok(source?.type === "system_event");
  assert.deepEqual(JSON.parse(source.detail!).files.map((f: {filename: string}) => f.filename), ["user.json", "chat.html"]);
...
test("rejects nonexistent ChatGPT children even with a disconnected root", () => {
  const mapping = {
    a: { id: "a", parent: null, children: ["missing"], message: { id: "a", author: { role: "user" }, content: { parts: ["a"] } } },
    b: { id: "b", parent: null, children: [], message: { id: "b", author: { role: "user" }, content: { parts: ["b"] } } },
  };
  assert.throws(() => importChatGptExport({ mapping }), /Missing ChatGPT child/);
});

test("preserves pasted boundary whitespace and tabs", () => {
  const result = importGenericTranscript("User:\tcode  \n\nAssistant: reply\n\n");
  const messages = result.record.events as MessageEvent[];
  assert.deepEqual(messages.map(e => e.content), [
    [{ type: "text", text: "\tcode  \n" }],
    [{ type: "text", text: "reply\n\n" }],
  ]);
  const plain = importGenericTranscript("\n  code  \n");
  assert.deepEqual((plain.record.events[0] as MessageEvent).content, [{ type: "text", text: "\n  code  \n" }]);
});
